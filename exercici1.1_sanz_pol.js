...
document.getElementById('bot').onclick = function generarInsult(){
    let llista1 = ['artless', 'bawdy', 'beslubbering', 'bootless', 'churlish', 'cockered', 'clouted', 'craven','currish',
    ...
    let llista2 = ['base-court', 'bat-fowling', 'beef-witted','beetle-headed','boil-brained','clapper-clawed','clay-brained','common-kissing',
    ...
    let llista3 = ['apple-john','baggage','barnacle','bladder','boar-pig','bugbear','bum-bailey','canker-blossom','clack-dish','clotpole','coxcomb',
    ...
    let llistafinal1 = llista1[Math.floor(Math.random()*llista1.length)];
    let llistafinal2 = llista1[Math.floor(Math.random()*llista2.length)];
    let llistafinal3 = llista1[Math.floor(Math.random()*llista3.length)];
    ...
    document.getElementById('esb').onclick = function esborrarInsult(){
        document.getElementById('sor').innerHTML=``;
    }
}